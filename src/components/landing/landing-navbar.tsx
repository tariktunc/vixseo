'use client'

import { useState, useEffect, useRef } from 'react'
import { usePathname } from 'next/navigation'
import Link from 'next/link'
import Image from 'next/image'
import { motion, AnimatePresence } from 'framer-motion'
import { SignInButton, SignUpButton, useAuth, UserButton } from '@clerk/nextjs'
import { Menu, X, LayoutDashboard, ChevronDown, Fingerprint, ShieldCheck, Accessibility, Palette, Search, Target, Globe, Bot, Gauge, Lock, Layout, Share2 } from 'lucide-react'
import { ThemeToggle } from '@/components/layout/theme-toggle'
import { Button } from '@/components/ui/button'

const serviceItems = [
  { slug: 'temel-kimlik', label: 'Temel Kimlik', icon: Fingerprint },
  { slug: 'yasal-uyumluluk', label: 'Yasal Uyumluluk', icon: ShieldCheck },
  { slug: 'erisilebilirlik', label: 'Erişilebilirlik', icon: Accessibility },
  { slug: 'tema-gorsel', label: 'Tema & Görsel', icon: Palette },
  { slug: 'seo', label: 'SEO', icon: Search },
  { slug: 'sem', label: 'SEM', icon: Target },
  { slug: 'geo', label: 'GEO', icon: Globe },
  { slug: 'aio-llmo', label: 'AIO/LLMO', icon: Bot },
  { slug: 'analitik-performans', label: 'Analitik & Performans', icon: Gauge },
  { slug: 'guvenlik', label: 'Güvenlik', icon: Lock },
  { slug: 'ux-temelleri', label: 'UX Temelleri', icon: Layout },
  { slug: 'sosyal-iletisim', label: 'Sosyal & İletişim', icon: Share2 },
]

const navLinks = [
  { label: 'Özellikler', href: '/features' },
  { label: 'Hizmetler', href: '/hizmetler', hasDropdown: true },
  { label: 'Nasıl Çalışır', href: '/how-it-works' },
  { label: 'Fiyatlar', href: '/pricing' },
  { label: 'Blog', href: '/blog' },
  { label: 'Araçlar', href: '/araclar' },
  { label: 'SSS', href: '/faq' },
]

export function LandingNavbar() {
  const [scrolled, setScrolled] = useState(false)
  const [mobileOpen, setMobileOpen] = useState(false)
  const [servicesOpen, setServicesOpen] = useState(false)
  const [mobileServicesOpen, setMobileServicesOpen] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)
  const timeoutRef = useRef<NodeJS.Timeout | null>(null)
  const { isSignedIn } = useAuth()

  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 20)
    window.addEventListener('scroll', onScroll, { passive: true })
    return () => window.removeEventListener('scroll', onScroll)
  }, [])

  const handleMouseEnter = () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current)
    setServicesOpen(true)
  }

  const handleMouseLeave = () => {
    timeoutRef.current = setTimeout(() => setServicesOpen(false), 200)
  }

  const pathname = usePathname()
  const forceDarkBg = pathname.startsWith('/docs') || pathname.startsWith('/privacy-policy') || pathname.startsWith('/terms-of-use') || pathname.startsWith('/kvkk') || pathname.startsWith('/dashboard') || pathname.startsWith('/settings')

  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || forceDarkBg
          ? 'bg-[#0F2447]/90 dark:bg-[#0B1120]/90 backdrop-blur-xl shadow-lg shadow-black/10 border-b border-white/5'
          : 'bg-transparent'
      }`}
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <Image src="/logo-dark.svg" alt="VixSEO" width={130} height={30} priority className="h-8 w-auto" />
        </Link>

        {/* Desktop Nav */}
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) =>
            link.hasDropdown ? (
              <div
                key={link.href}
                ref={dropdownRef}
                className="relative"
                onMouseEnter={handleMouseEnter}
                onMouseLeave={handleMouseLeave}
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 text-sm font-medium text-slate-300 hover:text-white transition-colors"
                >
                  {link.label}
                  <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${servicesOpen ? 'rotate-180' : ''}`} />
                </Link>

                <AnimatePresence>
                  {servicesOpen && (
                    <motion.div
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: 8 }}
                      transition={{ duration: 0.15 }}
                      className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[540px] rounded-xl bg-[#0B1528]/98 backdrop-blur-xl border border-white/10 shadow-2xl shadow-black/40 p-4"
                    >
                      <div className="grid grid-cols-2 gap-1">
                        {serviceItems.map((item, i) => {
                          const Icon = item.icon
                          return (
                            <Link
                              key={item.slug}
                              href={`/hizmetler/${item.slug}`}
                              onClick={() => setServicesOpen(false)}
                              className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-white/5 transition-colors group"
                            >
                              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-400 group-hover:bg-emerald-500/20 transition-colors shrink-0">
                                <Icon className="h-4 w-4" />
                              </span>
                              <span>
                                <span className="text-[10px] text-emerald-400/70 font-medium">Faz {i + 1}</span>
                                <span className="block text-sm leading-tight">{item.label}</span>
                              </span>
                            </Link>
                          )
                        })}
                      </div>
                      <div className="mt-3 pt-3 border-t border-white/10">
                        <Link
                          href="/hizmetler"
                          onClick={() => setServicesOpen(false)}
                          className="flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-emerald-400 hover:text-white hover:bg-emerald-500/10 transition-colors"
                        >
                          Tüm Hizmetleri Görüntüle →
                        </Link>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
            ) : (
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-slate-300 hover:text-white transition-colors"
              >
                {link.label}
              </a>
            )
          )}
        </nav>

        {/* Desktop Actions */}
        <div className="hidden md:flex items-center gap-3">
          <ThemeToggle className="text-slate-300 hover:text-white hover:bg-white/10" />
          {isSignedIn ? (
            <>
              <Link href="/dashboard">
                <Button
                  size="sm"
                  className="bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/25 gap-2"
                >
                  <LayoutDashboard className="h-4 w-4" />
                  Panele Git
                </Button>
              </Link>
              <UserButton
                appearance={{
                  elements: {
                    avatarBox: 'h-8 w-8',
                  },
                }}
              />
            </>
          ) : (
            <>
              <SignInButton mode="modal">
                <span
                  role="button"
                  tabIndex={0}
                  className="inline-flex h-9 items-center justify-center rounded-md px-3 text-sm font-medium text-slate-300 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                >
                  Giriş Yap
                </span>
              </SignInButton>
              <SignUpButton mode="modal">
                <span
                  role="button"
                  tabIndex={0}
                  className="inline-flex h-9 items-center justify-center rounded-md bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/25 px-3 text-sm font-medium transition-colors cursor-pointer"
                >
                  Ücretsiz Başla
                </span>
              </SignUpButton>
            </>
          )}
        </div>

        {/* Mobile Toggle */}
        <button
          className="md:hidden text-white p-2"
          onClick={() => setMobileOpen(!mobileOpen)}
          aria-label="Menü"
        >
          {mobileOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
        </button>
      </div>

      {/* Mobile Menu */}
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="md:hidden overflow-hidden bg-[#0F2447]/95 dark:bg-[#0B1120]/95 backdrop-blur-xl border-t border-white/10"
          >
            <div className="flex flex-col gap-1 px-4 py-6">
              {navLinks.map((link) =>
                link.hasDropdown ? (
                  <div key={link.href}>
                    <button
                      onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                      className="flex items-center justify-between w-full py-2 text-slate-300 hover:text-white transition-colors"
                    >
                      <span>{link.label}</span>
                      <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${mobileServicesOpen ? 'rotate-180' : ''}`} />
                    </button>
                    <AnimatePresence>
                      {mobileServicesOpen && (
                        <motion.div
                          initial={{ opacity: 0, height: 0 }}
                          animate={{ opacity: 1, height: 'auto' }}
                          exit={{ opacity: 0, height: 0 }}
                          className="overflow-hidden"
                        >
                          <div className="pl-4 pb-2 flex flex-col gap-1">
                            {serviceItems.map((item, i) => {
                              const Icon = item.icon
                              return (
                                <Link
                                  key={item.slug}
                                  href={`/hizmetler/${item.slug}`}
                                  onClick={() => { setMobileOpen(false); setMobileServicesOpen(false) }}
                                  className="flex items-center gap-3 py-2 text-sm text-slate-400 hover:text-white transition-colors"
                                >
                                  <Icon className="h-4 w-4 text-emerald-400" />
                                  <span className="text-[10px] text-emerald-400/70 mr-1">Faz {i + 1}</span>
                                  {item.label}
                                </Link>
                              )
                            })}
                            <Link
                              href="/hizmetler"
                              onClick={() => { setMobileOpen(false); setMobileServicesOpen(false) }}
                              className="py-2 text-sm text-emerald-400 hover:text-emerald-300 font-medium transition-colors"
                            >
                              Tüm Hizmetleri Gör →
                            </Link>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
                ) : (
                  <a
                    key={link.href}
                    href={link.href}
                    className="py-2 text-slate-300 hover:text-white transition-colors"
                    onClick={() => setMobileOpen(false)}
                  >
                    {link.label}
                  </a>
                )
              )}
              <div className="flex flex-col gap-2 pt-4 border-t border-white/10">
                {isSignedIn ? (
                  <Link
                    href="/dashboard"
                    className="flex items-center gap-2 text-white bg-emerald-500 hover:bg-emerald-600 rounded-md px-4 py-2 font-medium text-sm transition-colors"
                    onClick={() => setMobileOpen(false)}
                  >
                    <LayoutDashboard className="h-4 w-4" />
                    Panele Git
                  </Link>
                ) : (
                  <>
                    <SignInButton mode="modal">
                      <span
                        role="button"
                        tabIndex={0}
                        className="inline-flex h-10 w-full items-center justify-start rounded-md px-4 py-2 text-sm font-medium text-slate-300 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                      >
                        Giriş Yap
                      </span>
                    </SignInButton>
                    <SignUpButton mode="modal">
                      <span
                        role="button"
                        tabIndex={0}
                        className="inline-flex h-10 w-full items-center justify-center rounded-md bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 text-sm font-medium transition-colors cursor-pointer"
                      >
                        Ücretsiz Başla
                      </span>
                    </SignUpButton>
                  </>
                )}
              </div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
  )
}
